...
def main():
    ...
    # ── Dataset paths (loaded after tokenizer) ─────────────────────────────
    ...
    # ── Load + pre-format dataset with chat template ───────────────────────
    print(f"\n[Data] Loading + formatting with chat template")
    ...
    print(f"       Sample text length (chars): {len(dataset['train'][0]['text'])}")
    ...
    print(f"        Steps/epoch:     ~{len(dataset['train']) // (PER_DEVICE_BATCH * GRAD_ACCUM)}")
    print(f"        Total steps:     ~{NUM_EPOCHS * len(dataset['train']) // (PER_DEVICE_BATCH * GRAD_ACCUM)}")
